...
class WebsiteModel(
    ...
    val armVersion:String, 
    val buildScalaVersion:String, 
    val projectSites:List[ProjectSite],
    val outputDir:Path) {
  val self = this;
  def buildSite = {
    Printer.printPage(outputDir / "index.html",indexHtml)
    Printer.printPage(outputDir / "roadmap.html",Roadmap.html(this))
    Printer.printPage(outputDir / "getting-started.html",GettingStarted.html(this))
  ...
  def read(name:String) = {
    val file = (sourcePath ** name).head
    file.slurpString(Codec.UTF8)
  }

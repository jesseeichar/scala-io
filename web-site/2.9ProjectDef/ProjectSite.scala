import xml.Node
import scalax.file.Path.fromString

class ProjectSite(val project:SiteProject) {
  val self = this;
  ...
  val pages = project.samplesSources map {path => new ExamplesPage(fromString(path),self)}

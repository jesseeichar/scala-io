...
object Resource {
   ...
   *          an ReaderResource
   */
  def fromReader[A <: Reader](opener: => A)(implicit codec: Codec, extraCloser:Closer[A]=Noop) : ReaderResource[A] = new ReaderResource[A](opener, extraCloser, codec)
  /**
   ...
   */
  def fromBufferedReader[A <: BufferedReader](opener: => A)(implicit codec: Codec, extraCloser:Closer[A]=Noop): ReaderResource[A] = new ReaderResource[A](opener,extraCloser, codec) {
      override def buffered = this
  }
   ...
   *          an WriterResource
   */
  def fromWriter[A <: Writer](opener: => A)(implicit codec: Codec, extraCloser:Closer[A]=Noop) : WriterResource[A] = new WriterResource[A](opener,extraCloser, codec)
  /**
   ...
   */
  def fromBufferedWriter[A <: BufferedWriter](opener: => A)(implicit codec: Codec, extraCloser:Closer[A]=Noop): WriterResource[A] = new WriterResource[A](opener, extraCloser, codec) {
      override def buffered = this
  }
 ...
 * @see ManagedResource
 */
class ReaderResource[+A <: Reader](opener: => A, override protected val closer:Closer[A], val sourceCodec:Codec) extends BufferableReadCharsResource[A, BufferedReader] {
    def open() = opener
 ...
 * @see ManagedResource
 */
class WriterResource[+A <: Writer](opener: => A, override protected val closer:Closer[A], val sourceCodec:Codec) extends BufferableWriteCharsResource[A, BufferedWriter] {
    def open() = opener
